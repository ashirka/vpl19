        ...
        if all([i.isdigit() for i in coordinates]) and len(coordinates) == 2:
            ...
            if 1 <= x <= 3 and 1 <= y <= 3:
                if game_field[x - 1][y - 1] != CHAR_EMPTY:
                    print('Прости, но эти координаты {} {} уже заняты'.format(x, y))
                else:
                    return x - 1, y - 1
            else:
                print('Координаты должны быть числом от 1 до 3')
        else:
            print('Упс,Вы ввели неправильные координаты. Взгляни на пример: 1 3')


def get_side_map() -> dict:
    while True:
        value = input('Кто за кого будет играть {}? 1 - за "{}", 2 - за "{}": '.format(SIDE_HUMAN, CHAR_X, CHAR_O))
        side = value.strip()
        if side == '1':
            ...
            print('Так-так-так, Вы ввели неправильную сторону. Введите 1 или 2.')
...
if __name__ == '__main__':
    add_action('=== Игра всех веков и народов "Крестики-нолики" ===')
    add_action('Правом первого хода удостаиваются крестики.')
    ...
    if winner == CHAR_EMPTY:
        ...
        add_action('★ Победой награждается {} за "{}"! ★'.format({v: k for k, v in side_map.items()}[winner], winner))
    input()
